...
const Rooms: React.FC = () => {
  ...
  const handleSearchRooms = async (searchValue: string): Promise<void> => {
    if (searchValue !== "") {
      router.push(
        `/${locale}/admin/rooms/?page=1&size=10&keyword=${encodeURIComponent(searchValue)}`
      );
    } else {
      router.push(`/${locale}/admin/rooms/?page=1&size=10`);
    }
      ...
      <div className="w-full h-full">
        <div className="w-full h-[50px] flex items-center gap-2 md:gap-0 justify-between">
          <Input
            ...
            className="!w-[450px]"
            onChange={(e) => {
              handleSearchRooms(e.target.value);
            }}
          />
          <Button
            onClick={() => {
